    ...
    if (m_xi2detected) {
        ...
        if (m_impl->XGetEventData(m_display, cookie) &&
            ...
            cookie->extension == xi_opcode) {
                ...
                if (!m_isPrimary &&                                 // If on the client...
                        (
                            cookie->evtype == XI_RawMotion ||
                            cookie->evtype == XI_RawButtonPress ||
                            cookie->evtype == XI_RawKeyPress ||
                            cookie->evtype == XI_RawTouchBegin      // and motion or key/button presses or touch input is detected...
                        ) &&
                        m_screensaverNotificationTimer.getTime() > NOTIFICATION_TIMEOUT // and the server hasn't been notified in a while...
                    ) {
                        // then notify the server to suppress the screensaver by deactivating it
                        LOG((CLOG_INFO "Reset timer with value %f", m_screensaverNotificationTimer.getTime()));
                        m_screensaverNotificationTimer.reset();
                        m_events->addEvent(Event(m_events->forIPrimaryScreen().screensaverDeactivated(), getEventTarget()));
                }

                if (m_isPrimary && cookie->evtype == XI_RawMotion) {
